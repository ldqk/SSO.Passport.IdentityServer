...
namespace SSO.Passport.IdentityServer.Controllers
{
    public class FileController : BaseController
    {
        ...
        {
            var files = Directory.GetFiles(Request.MapPath(path)).OrderByDescending(s => s).Select(s => new { filename = Path.GetFileName(s), path = s }).ToList();
        ...
        public ActionResult Handle(FileRequest req)
        {
            ...
            var prefix = "";//todo:CommonHelper.GetSettings("PathRoot").Trim('\\', '/');
            switch (req.Action)
            {
                case "list":
                    ...
                    dirs.ForEach(s =>
                    {
                        ...
                        list.Add(new FileList() { date = dirinfo.LastWriteTime.ToString("yyyy-MM-dd HH:mm:ss"), name = dirinfo.Name, rights = "drwxrwxrwx", size = 0, type = "dir" });
                    });
                    files.ForEach(s =>
                    {
                        FileInfo info = new FileInfo(s);
                        list.Add(new FileList() { name = info.Name, rights = "-rw-rw-rw-", size = info.Length, date = info.LastWriteTime.ToString("yyyy-MM-dd HH:mm:ss"), type = "file" });
                    });
                    break;
                case "remove":
                    req.Items.ForEach(s =>
                ...
                case "extract":
                    ...
                    Masuit.Tools.Files.ClassZip.UnZip(zip, folder);
                    list.Add(new { success = "true" });
                    break;
                default:
                ...
                case "download":
                    ...
                    if (System.IO.File.Exists(file))
                    {
                        using (FileStream stream = System.IO.File.OpenRead(file))
                        {
                            ...
                            return File(buff, "application/octet-stream", Path.GetFileName(file));
                        }
                    }
                    break;
