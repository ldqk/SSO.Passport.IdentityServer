    ...
    internal sealed class Configuration : DbMigrationsConfiguration<DataContext>
    {
        ...
        protected override void Seed(DataContext context)
        {
            ...
            if (!context.UserInfo.Any())
            {
                ...
                List<ClientApp> apps = new List<ClientApp>() { new ClientApp() { AppName = "LocalSystem", AppId = appid, AppSecret = appid.MDString(DateTime.Now.GetTotalMilliseconds().ToString()), Description = "主系统应用程序" } };
                ...
                {
                    new Menu(){Name = "控制面板",CssStyle = "zmdi-home",RouteName = "dashboard",Route = "dashboard",ClientApp = apps.FirstOrDefault()},
                    new Menu(){Name = "子系统管理",CssStyle = "zmdi-card-travel",RouteName = "apps",Route = "apps",ClientApp = apps.FirstOrDefault()},
                    new Menu(){Name = "用户管理",CssStyle = "zmdi-assignment-account",RouteName = "user",Route = "user",ClientApp = apps.FirstOrDefault()},
                    new Menu(){Name = "用户组管理",CssStyle = "zmdi-flower-alt",RouteName = "group",Route = "group",ClientApp = apps.FirstOrDefault()},
                    new Menu(){Name = "角色管理",CssStyle = "zmdi-collection-text",RouteName = "role",Route = "role",ClientApp = apps.FirstOrDefault()},
                    new Menu(){Name = "权限管理",CssStyle = "zmdi-globe-lock",RouteName = "permission",Route = "permission",ClientApp = apps.FirstOrDefault()},
                    new Menu(){Name = "访问控制管理",CssStyle = "zmdi-flash-auto",RouteName = "access",Route = "access",ClientApp = apps.FirstOrDefault()},
                    new Menu(){Name = "菜单管理",CssStyle = "zmdi-menu",RouteName = "menu",Route = "menu",ClientApp = apps.FirstOrDefault()},
                    new Menu(){Name = "系统设置",CssStyle = "zmdi-wrench",RouteName = "system",Route = "system",ClientApp = apps.FirstOrDefault()},
                    new Menu(){Name = "文件管理",CssStyle = "zmdi-archive",RouteName = "filemanager",Route = "filemanager",ClientApp = apps.FirstOrDefault()},
                    new Menu(){Name = "任务管理器",CssStyle = "zmdi-view-list",RouteName = "taskcenter",Route = "taskcenter",ClientApp = apps.FirstOrDefault()}
                };
                ...
                IList<Permission> ps = new List<Permission>()
                {
                    ...
                    new Permission() {PermissionName = "主系统系统设置",Controls = controls.Where(c => c.Controller.Equals("System")).ToList(),Menu = menus.Where(m => m.Name.Equals("系统设置")).ToList(), ClientApp = apps,Description = "主系统系统设置"},
                ...
                {
                    new UserInfo() { Username = "admin", Password = "admin".MDString2(salt), SaltKey = salt, Email = "admin@masuit.com", PhoneNumber = "15205201520", ClientApp = apps,AccessKey = Guid.NewGuid().ToString().MDString(),IsMaster = true,IsPreset = true}
                };
